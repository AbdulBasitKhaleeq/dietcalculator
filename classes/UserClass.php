...
class UserClass{
    ...
    */
    ...
    require ("config/dbconnect.php");
        ...
        if(isset($_POST['login'])){
            ...
            if(!empty($user) && !empty($userpsw)){
                ...
                $sql = "SELECT id, username, password_d FROM users WHERE username = '$user'";
                $result = mysqli_query($conn, $sql);

                if (mysqli_num_rows($result) == 1) {
                    $row = mysqli_fetch_assoc($result);
                    // Cross-reference password that is given by user with the hashed password in database.
                    if (password_verify($userpsw, $row['password_d'])) {
    ...
    /* Function isNewUser()
     * Check if user is already made plan.
     */
    ...
    private function updatePassword(){
        ...
        if($password3===$password2){
            ...
            if(!empty($password3) && !empty($email)){
                ...
                $stmt = $conn->prepare("UPDATE users SET password_d = ?, fpassword_key = ?  WHERE email = ? AND fpassword_key = ?");
                $clean_password_key = "";
            ...
            if($password===$password2){
                ...
                if(!empty($username) && !empty($password) && !empty($email)){
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        $sql = "INSERT INTO users (username, email, password_d, activation_code) VALUES ('$username', '$email', '$securing', '$code')";
                        ...
                        // $from = 'a.basit.k08@gmail.com';  // This should be changed to an email that you would like to send activation e-mail from.
